...
def generate_answer(prompt: str) -> str:
    try:
        ...
        response = requests.post(url, json=payload, timeout=10) # Send request to Ollama
        response.raise_for_status() # Check for HTTP errors
        data = response.json() # Parse JSON response
        return data.get("response", "").strip() # Extract and return the generated answer
    except Exception:
        return "I could not reach the local LLM service, so I used a fallback response. Please ensure Ollama is running if you want model-backed answers."